    ...
    var dialogObj = {
        ...
        init: function (options) {
            options = $.extend({
                ...
                //left: 50,
                infoBg: "green",
                isAutoClose: true,//info框是否自动关闭
                closeBtn: true,
                btnList: null,
                beforeOk: null,
                onOk: null,
                onCancel: null
//              closePlug:this.close
            }, options);
            this.config = options;
            var that = this;
            /*//加载样式
        ...
        initDom: function () {
            ...
            //创建文档碎片
            container = $("<div class='plugin_dialog'><div class='plugin_dialog_bg'></div><div class='plugin_dialog_content'><div class='plugin_dialog_hd'><div class='plugin_dialog_logotop'><div class='title_box'><p id='title'></p></div></div><a href='javascript:' class='closeBtn close' title='关闭'></a></div><div class='content plugin_dialog_bd'></div><div class='standard Btn'><button class='send'  type='submit'>确定</button><button class='cancelBtn send2 close' type='button'>取消</button></div></div></div>");
            //右上角关闭按钮是否存在
            ...
            switch (config.dialogType) {
                case "alert":
                    ...
                    break;
                case "info":
                    container = $("<div class='plugin_dialog'><div class='plugin_dialog_bg'></div><div class='plugin_dialog_content plugin_dialog_contentInfo'><div class='plugin_dialog_hd'><div class='plugin_dialog_logotop'><div class='title_box'><p id='title'></p></div></div><a href='javascript:' class='closeBtn close' title='关闭'></a></div><div class='content plugin_dialog_bd'></div><div class='standard Btn'><button class='send'  type='submit'>确定</button><button class='cancelBtn send2 close' type='button'>取消</button></div></div></div>");
                    //信息框 隐藏标题和按钮
                    container.find(".content,.Btn").hide();
                    container.find(".plugin_dialog_content").addClass("plugin_dialog_personal");
                    container.find(".plugin_dialog_hd").addClass(config.infoBg);
                ...
                case "examine":
                    ...
                    //隐藏关闭按钮、遮罩层
                   // container.find(".plugin_dialog_bg,.send2").hide();
                   container.find(".send").text("通过");
                    container.find(".send2").text("拒绝");
                    break;
                case "message":
                    //信息框 隐藏标题和按钮
           ...
           //设置title内容
            ...
            //设置宽高和默认上下左右居中
            var $doc;
            //info类型success or failure
            if (this.config.dialogType == "info") {
                $doc = $('.plugin_dialog_contentInfo'); //获取元素暂存
            } else {
                $doc = $('.plugin_dialog_content'); //获取元素暂存
            }
        ...
        initEvents: function () {
            var that = this;
            //关闭、取消
            that.container.find(".close").on("click.close",function () {
                that.container.remove();
        ...
        moveDialog: function () {
            ...
            $(document).mousemove(function (e) {
                if (!isMoving) return;
                //根据鼠标移动情况设置对话框位置
                var contentDom = that.container.find(".plugin_dialog_content") || that.container.find(".plugin_dialog_contentInfo");
